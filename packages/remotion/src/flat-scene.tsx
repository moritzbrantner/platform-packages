...
import {
  ...
  compileFlatMotion,
  type FlatAnimation,
  type FlatAttributeAnimation,
  type FlatDesignScene,
  type FlatSceneProps,
  type FlatShape,
  type FlatTransformAnimation,
...
export function sampleFlatSceneAtTime(scene: FlatDesignScene, timeInMs: number): FlatDesignScene {
  return {
    ...
    layers: scene.layers.map((layer) => ({
      ...layer,
      shapes: layer.shapes.map((shape) => sampleFlatShapeAtTime(shape, timeInMs)),
    })),
  };
}
...
function sampleFlatShapeAtTime(shape: FlatShape, timeInMs: number): FlatShape {
  const animations = getShapeAnimations(shape);
...
function sampleFlatAnimationAtTime(
  ...
  const value = sampleFlatTransformAnimation(animation, timeInMs);
...
function sampleAnimationValues<TValue>(
  ...
  const progress = getAnimationProgress(animation, timeInMs);
...
function getAnimationProgress(animation: FlatAnimation, timeInMs: number) {
  ...
  const iterationElapsed =
    repeatCount === "indefinite" ? modulo(elapsed, durationInMs) : modulo(elapsed, durationInMs);
