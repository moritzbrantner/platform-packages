...
function AvatarCollection({
  ...
  const hiddenCount = maxVisible !== undefined ? Math.max(0, users.length - visibleUsers.length) : 0;
  ...
  return (
    ...
    >
      {visibleUsers.map(({ key, ...user }, index) => (
        <Avatar key={key ?? user.name ?? user.initials ?? user.imageUrl ?? `avatar-${index}`} {...user} />
      ))}
      {hiddenCount > 0 ? (
