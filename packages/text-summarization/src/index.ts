...
export function createTextSummarizationPipeline<
  Metadata extends Record<string, unknown> = Record<string, unknown>,
>(options: CreateTextSummarizationPipelineOptions<Metadata>): TextSummarizationPipeline<Metadata> {
  return {
    async summarize(input, summarizeOptions = {}) {
      const maxPasses = Math.max(
        1,
        Math.floor(summarizeOptions.maxPasses ?? options.maxPasses ?? DEFAULT_MAX_PASSES),
      );
      ...
      while (pass < maxPasses) {
        const chunks = chunkTextForInference(
          currentInput,
          summarizeOptions.chunking ?? options.chunking,
        );
        ...
        const nextChunks = chunkTextForInference(
          joinedSummary,
          summarizeOptions.chunking ?? options.chunking,
        );
